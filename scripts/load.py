                ...
                # if obligatoire : 
                    if type == 'text':
                        ...
                        if default == "today":
                            ...
                            value = datetime.today().strftime("%H%M%S") 
                # else :
                #     if name == "date_production" : 
                #         value = datetime.today().strftime("%Y%m%d")
                #     if name == "Heure_production" :
                #         value = datetime.today().strftime("%H%M%S") 
                #     else : value = " " * length

            # Make sure that length is the required length
            if len(value) < length :
                if type == "integer": 
                    value = value.zfill(length)
                else:
...
#         lines.append(generate_simt_line(structure["Header"]["Fields"], data_row=row))
...
def generate_simt_file(yaml_path, df, extension="txt", month=None, year=None):
    ...
    # --- FILTRAGE PAR MOIS ET ANNEE SANS MODIFIER LE DF ORIGINAL ---
    ...
    if "reference_remise" in df_filtered.columns and not df_filtered.empty:
        ...
        # # Nettoyage pour éviter les caractères invalides dans le nom de fichier
        # reference_remise = reference_remise.replace('\n', '').replace('\r', '').replace(' ', '').replace('/', '').replace('\\', '')
    else:
